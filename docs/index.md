...
При этом лишь ```16.5TP``` из ```24TP``` являются базовой частью (которая в любом случае должна быть выполнена), а оставшиеся ```7.5TP``` могут быть использованы для доразработки задач для которых нет полного архива, а также для составления разборов.
...
| Есть генератор, но тестов нет | 10% стоимости задачи     |
| Нет авторского | 20% стоимости задачи     |
| Отсутствует кастомный чекер    | 0.75TP   |

Эта работа выполняется до 23:59-UTC+3/MSC среды.

#### Технические требования к задачам

- Каждый тест должен принадлежать ровно одной группе
- Условия должны рендериться в PDF и HTML
- Подгруппы должны именоваться числами, то есть 0, 1, 2, ...
- В каждой группе EACH_TEST скоринг тестов должен быть одинаковым
- Feedback policy должен быть равен ICPC для всех групп
- Зависимости групп должны быть расставлены
- Должен собираться полный пакет с верификацией

#### Полезные советы

- В полигоне есть возможность выбирать несколько тестов, для этого надо выбрать первый тест группы, зажать шифт и выбрать последний
- Для загрузки целого пакета с problem.xml удобно использовать [vydra](https://github.com/Gornak40/algolymp#vydra)
- Для загрузки задач по кускам удобно использовать [wooda](https://github.com/Gornak40/algolymp#wooda)
- Для загрузки задач без авторского удобно использовать [pepel](https://github.com/Gornak40/algolymp#pepel)
- Для генерации тех таблички с баллами удобно использовать [valeria](https://github.com/Gornak40/algolymp#valeria)
...
### Разборы
Начиная с 2 ноября мы ко всем задачами дистура также публикуем разборы. К разборам мы предъявляем соответствующие требования:

- Разборы пишем ко всем значимым группам задачи
- Разборы пишем используя TeX, и так чтобы его было приятно читать

За составление разбора добавляется бонус.
| Ситуация    | Бонус |
| -------- | ------- |
| Разбор есть и его можно просто вставить в polygon | 0TP   |
| Разбор есть, но нужно ощутимо дотехать его | 0.3TP    |
| Разбор есть, но нужно ощутимо дотехать его и перевести | 1TP    |
| Разбора совсем нет | 1.5TP    |

При этом, если разбора в задаче совсем нет и вы не хотите его писать, то при условии, что задача полностью согласована ревьювером до вторника 23:59, вы можете запросить передачу его написания на другого человека. Такое действие требует согласования у @CoffeBee. Кто будет писать разбор, выбирается тем же алгоритмом, что и для остальных видов работ; стоимость возрастает до 2TP.
...
1. Задача не подходит, мы неготовы ее дать на дистур. В этом случае нужно доходчево описать почему задача не подходит, сославший на один из вариантов: слишком простая/сложная, баян, слишком учебная или не подходящий формат. **Откланено**
2. Задача подходит на свою позицию, но задача скучная или звучит искуствено. **Нормально**
